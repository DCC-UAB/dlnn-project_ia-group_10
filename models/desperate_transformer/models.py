...
class desperate_transformer(nn.Module):
    ...
    def forward(self, x, x2):
        ...
        tok = torch.tensor([[1]]).to("mps")
        
        caption = torch.tensor([]).to("mps")
        #ic(hidden.shape)
        for a in range(hidden.shape[1]):
            h_b = hidden[:,a,:]
            #ic(h_b.shape)
            b_caption = torch.tensor([]).to("mps")
            h,c = h_b.unsqueeze(0), h_b.unsqueeze(0)
            for i in range(x2.shape[1]):
                out,(h,c) = self.lstm(self.embedding(tok),(h,c))
                h,c = h*h_b.unsqueeze(0),c
                out = self.LM_FC(out)
                out_s = nn.functional.softmax(out,dim=-1)
                gen_word = torch.multinomial(out_s[0,0], 1)
                b_caption = torch.cat((b_caption,out),axis=1)
                tok = gen_word.unsqueeze(0) 

            caption = torch.cat((caption,b_caption),axis=0)    
